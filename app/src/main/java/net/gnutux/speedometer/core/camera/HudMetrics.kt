...
object HudMetrics {
    ...
    /**
     * عتبة الخطر حين **لا حدَّ** مضبوطًا: نسبةٌ من أقصى القرص. وعتبة التحذير
     * `warnKmh` تأتي مع اللقطة. أمّا حين يضبط السائق حدًّا فالحكم للحدّ وحده،
     * وموضعه [net.gnutux.speedometer.core.alert.SpeedScale.zoneOf].
     */
    const val DANGER_OF_MAX = 0.92f

    /**
     * علامة الحدّ: خطٌّ شعاعيّ أحمر عند حدّ السائق على مسار القوس.
     *
     * تبدّلُ اللون وحده لا يكفي: السائق يعرف أنّه تجاوز بعد أن يتجاوز، والعلامة
     * تُريه **أين** الحدّ قبل بلوغه فيهدّئ قبله. وهي نِسَبٌ لا مقاسات، كسائر ما في
     * هذا العقد، كي يخرج المرسوم والمحروق واحدًا.
     *
     * العرض نسبةٌ من سماكة القوس لا من الضلع: قوسٌ أسمك يحتمل علامةً أعرض. وهي عند
     * ‎0.34‎ أعرضُ من علامات التدريج (‎2dp‎) في كلّ المقاسات المعقولة.
     * والطول ‎1.5‎ من السماكة كي تنتأ عن القوس من طرفيه فتُقرأ علامةً لا فجوةً فيه.
     */
    const val LIMIT_MARK_WIDTH_OF_STROKE = 0.34f
    const val LIMIT_MARK_LENGTH_OF_STROKE = 1.5f
