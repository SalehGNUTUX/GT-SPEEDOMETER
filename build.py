...
FEATURES: list[tuple[str, str, list[tuple[str, str]]]] = [
    ("القياس", "speed", [
        ...
        ("خمس نغماتٍ وشدّةٌ تُضبط",
         "صفيرة · نبضتان · جرس · رنّة رقميّة · نبضة خفيفة، مع زرّ «استمع» يشغّلها من مسار التنبيه نفسه لا من مسارٍ ثانٍ."),
        ("ستّة تصاميم للقرص",
         "كلاسيكيّ · مؤشّر تناظريّ · مقتضب · شرات مضيئة · حلقتان · شريط. تُختار بمنتقٍ <strong>يُري التصميم لا اسمه</strong>، والمدى والعتبة والحدّ واحدةٌ فيها جميعًا."),
    ]),
    ("الكاميرا", "cam", [
    ...
    ("المسار والخرائط", "map", [
        ...
        ("مصدر الخريطة قائمةٌ منسدلة",
         "تلقائيّ · إنترنت · OsmAnd · محلّيّة. وما لا يعمل الآن يُعرض معطَّلًا ومعه سببه، لا يُخفى."),
        ("قائمةٌ بما في مجلّد الخرائط",
         "كلّ ملفٍّ باسمه وحجمه وتصنيفه — أرشيفٌ يُرسم أم متجهيّةٌ أم بياناتٌ خام — مع مشاركته أو حذفه. والحذف مقصورٌ على مجلّدنا: ما وُجد عند تطبيقٍ آخر يُقرأ ولا يُمسّ."),
        ("لا يُدّعى خريطةً ما ليس خريطة",
         "ملفّات Geofabrik و‎.shp.zip‎ و‎.gpkg‎ تُفتح فهارسُها ويُبحث فيها عن بلاطةٍ حقيقيّة، فإن لم تُوجد صُنّفت بياناتٍ خامّة وقيل ذلك صراحةً."),
        ("جسرٌ إلى OsmAnd",
...
<script>
  /* نقرُ الشعار: إلى أعلى الصفحة **ثمّ** إنعاشها.
     كان `scrollTo(0,0)` وحده لا يكفي: المتصفّح يحفظ موضع التمرير ويعيده بعد
     الإنعاش (`history.scrollRestoration` وقيمتها `auto`)، فتُنعش الصفحة وتعود
     إلى حيث كان الزائر. فتُطفأ الاستعادة أوّلًا، وحينئذٍ يبدأ الإنعاش من الأعلى.
     ونُطفئها عند التحميل لا عند النقر وحده: من أنعش بمفتاح F5 يستحقّ السلوك نفسه.
     و`href="#top"` يبقى عاملًا بلا نصٍّ برمجيّ أصلًا. */
